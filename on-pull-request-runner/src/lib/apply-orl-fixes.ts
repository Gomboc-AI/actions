...
function pathsFromRule(rule: OrlReportRule): string[] {
...
export function pathsFromReport(report: OrlReport | null): string[] {
  ...
  for (const rule of report.spec?.rules ?? []) {
    for (const p of pathsFromRule(rule)) paths.add(p);
  }
  return [...paths].sort();
}
...
export function applyOrlFixes(args: ApplyOrlFixesArgs): ApplyOrlFixesResult {
  ...
  for (const batch of batches) {
    ...
    const targetPaths =
      reportPaths.length > 0 ? reportPaths : manifestPaths.map(normalizeRepoPath);
