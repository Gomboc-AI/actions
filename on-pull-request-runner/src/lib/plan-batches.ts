...
export function deepestWorkspaceForFile(
  ...
  for (const ws of workspaces) {
    if (!isUnderPath({ filePath, dirPath: ws.workspacePath })) continue;
    const depth =
      ws.workspacePath === '.'
        ? 0
...
export function buildEvaluationBatches(
  args: BuildEvaluationBatchesArgs
): EvaluationBatch[] {
  const { scannableFiles, workspaces, resolveLanguage, warn = console.warn } = args;
  const batchMap = new Map<
    string,
    { workspacePath: string; orlLanguage: string; files: string[] }
  >();
