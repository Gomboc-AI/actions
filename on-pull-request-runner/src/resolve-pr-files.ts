...
async function main(): Promise<void> {
  ...
  if (mode === 'remediate') {
    throw new Error(
      'mode=remediate is not available yet. Use mode: audit for Phase 1.'
    );
  }
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ paths: changed }, null, 2)
  );

  if (changed.length > maxFiles) {
    throw new Error(
      `PR changes ${changed.length} paths exceeds max-changed-files (${maxFiles}). Split the PR or raise the limit.`
    );
  }

  const scannable = changed.filter((p) =>
    isScannable({ filePath: p, workspaceRoot: workspace })
  );
  fs.writeFileSync(
    artifactPath('pr-scannable-files.json'),
    JSON.stringify({ files: scannable }, null, 2)
  );
  ...
  fs.writeFileSync(
    artifactPath('touch-seeds.json'),
    JSON.stringify({ seeds }, null, 2)
  );

  setOutput('skip', 'false');
  console.log(
    `PR scope: ${changed.length} changed path(s), ${scannable.length} scannable, ${seeds.length} touch seed(s)`
  );
}
