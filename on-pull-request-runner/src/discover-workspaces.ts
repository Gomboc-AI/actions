...
import type { DetectLanguageEntry, TouchedWorkspace } from './types.js';

type DetectLanguageMap = Record<string, DetectLanguageEntry[]>;
...
async function main(): Promise<void> {
  ...
  const { seeds } = loadJson<{ seeds: string[] }>(artifactPath('touch-seeds.json'));
  const { files: scannable } = loadJson<{ files: string[] }>(
    artifactPath('pr-scannable-files.json')
  );
  const { paths: changed } = loadJson<{ paths: string[] }>(
    artifactPath('pr-changed-paths.json')
  );

  const workspaceMap = new Map<string, DetectLanguageEntry[]>();
  ...
  for (let i = 0; i < seeds.length; i++) {
    ...
    const map = loadJson<DetectLanguageMap>(outFile);
  ...
  const touched: TouchedWorkspace[] = [];
  for (const [workspacePath, languages] of workspaceMap) {
    const relevantChanged = [...changed, ...scannable].filter((p) =>
      isUnderPath({ filePath: p, dirPath: workspacePath })
    );
  ...
  fs.writeFileSync(
    artifactPath('touched-workspaces.json'),
    JSON.stringify({ workspaces: touched }, null, 2)
  );
