...
async function main(): Promise<void> {
  ...
  let copiedPaths: string[] = replay.allFiles;
  ...
  if (replay.commitCount === 0) {
    ...
    if (applied.copiedPaths.length) {
      console.log(
        `Applied ORL fixes for ${applied.copiedPaths.length} path(s): ${applied.copiedPaths.join(', ')}`
      );
    } else {
      console.log('No remediated files copied from batch workspaces');
      if (applied.skippedUnchanged.length) {
        console.log(
          `${applied.skippedUnchanged.length} staged path(s) unchanged vs checkout: ${applied.skippedUnchanged.slice(0, 10).join(', ')}${applied.skippedUnchanged.length > 10 ? '…' : ''}`
        );
      }
      if (applied.skippedMissing.length) {
        console.log(
          `${applied.skippedMissing.length} candidate path(s) missing in batch workspace: ${applied.skippedMissing.slice(0, 10).join(', ')}${applied.skippedMissing.length > 10 ? '…' : ''}`
        );
      }
    }
    ...
    console.log(
      `Replayed ${replay.commitCount} ORL hook commit${replay.commitCount === 1 ? '' : 's'} onto ${botBranch}`
    );
  }

  gitPush('origin', botBranch, workspaceRoot);
  console.log(
    `Pushed remediation branch ${botBranch} (${replay.commitCount} commit${replay.commitCount === 1 ? '' : 's'})`
  );
