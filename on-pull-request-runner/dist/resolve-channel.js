...
import { encodedChannelPath, portalChannelUrl, portalPolicySetUrl, } from './lib/portal-url.js';
import { runMain } from './lib/runner.js';
function setChannelOutputs(channel) {
    setOutput('channel', channel);
    setOutput('encoded-channel', encodedChannelPath(channel));
}
function formatChannelLogLine(label, channel, portalServiceUrl) {
...
async function main() {
    ...
    if (inputChannel) {
        ...
        console.log(formatChannelLogLine('Using orl-channel input', inputChannel, portalServiceUrl));
        return;
    }
    ...
    if (!tenantId) {
        ...
        console.log(formatChannelLogLine('Resolved rules channel', DEFAULT_CHANNEL_NAME, portalServiceUrl));
        return;
    }
    ...
    console.log(formatChannelLogLine('Resolved rules channel', channel, portalServiceUrl));
}
runMain(main);
