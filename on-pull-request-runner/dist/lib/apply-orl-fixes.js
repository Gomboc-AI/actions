...
function pathsWithChangesFromRule(rule, workspacePath) {
    const paths = new Set();
    for (const raw of Object.keys(rule.files_changed ?? {})) {
        paths.add(reportPathToRepoPath({ reportPath: raw, workspacePath }));
    }
    if ((rule.fixes ?? 0) > 0 || (rule.changes ?? 0) > 0) {
        for (const file of rule.files ?? []) {
            if (file.path) {
                paths.add(reportPathToRepoPath({ reportPath: file.path, workspacePath }));
            }
        }
        for (const row of rule.finding_locations ?? []) {
            const loc = row.resolved_location ?? row.original_location;
            if (loc?.file_path) {
                paths.add(reportPathToRepoPath({ reportPath: loc.file_path, workspacePath }));
            }
        }
    }
    return [...paths];
...
export function pathsWithChangesFromReport(report, workspacePath) {
    if (!report)
        return [];
    const paths = new Set();
...
export function applyOrlFixes(args) {
    ...
    const skippedUnchanged = new Set();
    const skippedMissing = new Set();
    for (const batch of batches) {
        const workDir = path.join(batchWorkRoot, batch.batchId);
        if (!fs.existsSync(workDir))
            continue;
        const reportPaths = pathsWithChangesFromReport(reportForBatch(batch.batchId), batch.workspacePath);
        const manifestPaths = (stagedFilesForBatch(batch.batchId) ?? []).map(normalizeRepoPath);
