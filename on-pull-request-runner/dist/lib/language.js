...
import { detectLanguageId, isOrlScannableLanguageFile, mapLanguageIdToOrlLanguage, } from '@gomboc-ai/gomboc-node-sdk';
/** Reads file text from the consumer workspace; returns empty string if missing. */
export function readFileContent(args) {
...
export function isScannable(args) {
    const { filePath, workspaceRoot } = args;
...
export function orlLanguageForFile(args) {
    const { filePath, workspaceRoot } = args;
    const content = readFileContent({ filePath, workspaceRoot });
